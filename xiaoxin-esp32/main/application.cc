...
void Application::Start() {
    ...
    auto codec = board.GetAudioCodec();
    ESP_LOGI(TAG, "Audio codec: %p", codec);
    opus_decoder_ = std::make_unique<OpusDecoderWrapper>(codec->output_sample_rate(), 1, OPUS_FRAME_DURATION_MS);
    ESP_LOGI(TAG, "opus_decoder_: %p", opus_decoder_.get());
    opus_encoder_ = std::make_unique<OpusEncoderWrapper>(16000, 1, OPUS_FRAME_DURATION_MS);
    if (opus_encoder_) {
        ESP_LOGI(TAG, "opus_encoder_ 创建成功");
        ESP_LOGI(TAG, "opus_encoder_: %p", opus_encoder_.get());
    } else {
        ESP_LOGE(TAG, "opus_encoder_ 创建失败");
    }
    ...
    if (codec->input_sample_rate() != 16000) {
        ...
        ESP_LOGI(TAG, "input_resampler_: %p", &input_resampler_);
        reference_resampler_.Configure(codec->input_sample_rate(), 16000);
        ESP_LOGI(TAG, "reference_resampler_: %p", &reference_resampler_);
    }
    ESP_LOGI(TAG, "Starting codec...");
    if (codec != nullptr) {
    ...
    protocol_->OnAudioChannelOpened([this, codec, &board]() {
        ...
        std::string json = thing_manager.GetDescriptorsJson();       // 获取 string

        ESP_LOGI(TAG, "thing_manager JSON: %s", json.c_str());       // 打印日志
